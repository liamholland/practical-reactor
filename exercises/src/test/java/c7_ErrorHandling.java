...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void under_the_rug() {
        Flux<String> messages = messageNode()
            .onErrorResume(e -> Mono.empty())   //why no onErrorComplete like in the docs? Does not seem to exist
        ;
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
                ...
                .onErrorContinue((e, file) -> Function.identity())
                ;
    ...
    public void resilience() {
        ...
        Flux<String> content = getFilesContent()
                .flatMap(file -> file.onErrorResume(e -> Mono.empty()));
    ...
    public void good_old_polling() {
        //todo: change code as you need
        Flux<String> alerts = nodeAlerts().repeat().switchIfEmpty(Flux.interval(Duration.ofSeconds(1)).empty());
