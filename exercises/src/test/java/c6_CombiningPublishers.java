...
public class c6_CombiningPublishers extends CombiningPublishersBase {
     ...
     *
     * Answer:
     * - What is difference between concatMap() and flatMap()?  concatenation vs merging
     * - What is difference between concatMap() and flatMapSequential()?    waits for completion of each before next subscription vs "eagerly" subscribes but maintains order
     * - Why doesn't Mono have concatMap() operator?    no need
     */
    @Test
    ...
    public void mail_box_switcher() {
        ...
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) -> {
            if(signal.hasValue()){
                if(signal.get().metaData.equals("spam")) return mailBoxSecondary();
            }
            return flux;
        });
    ...
    public void car_factory() {
        //todo: feel free to change code as you need
        Flux<Car> producedCars = Flux.zip(carChassisProducer(), carEngineProducer()).cast(Car.class);
    ...
    public Mono<String> chooseSource() {
        return Mono.just(sourceRef.get())
            .flatMap(source -> {
                if(source.equals("A")){
                    return sourceA();
                }
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(
            ...
            Function.identity(),
            i -> StreamingConnection.closeConnection());
